...
professor_trash_wheel = 
  ...
  janitor::clean_names(.) %>%
  mutate(year = as.character(year), sports_balls = 0, trash_program = "professor_trash_wheel") %>% 
  filter(!is.na(dumpster))
```
...
```
```{r}
sum(tidy_trash_wheel$weight_tons[tidy_trash_wheel$trash_program == "professor_trash_wheel"])
```
```{r}
sum(tidy_trash_wheel$sports_balls[tidy_trash_wheel$trash_program == "mr_trash_wheel" & tidy_trash_wheel$year == "2020"])
```
